...
if [[ -n "$cpu_only" ]]; then
    ...
    # on Linux, rename the package pytorch-cpu, because it's cpu build
    if [[ "$(uname)" != 'Darwin' ]]; then
        $portable_sed "s/name: pytorch/name: pytorch-cpu/" "$meta_yaml"
    fi
else
    ...
    if [[ "$build_folder" == 'pytorch-1.0.0' && "$desired_cuda" != '9.0' ]]; then
