...
if [[ "$package_type" == conda || "$(uname)" == Darwin ]]; then
    ...
    if [[ "$cuda_ver" != 'cpu' ]]; then
        retry conda install -c numba/label/dev -yq cudatoolkit=$cuda_ver_majmin
    else
    ...
    retry conda install -yq cffi future hypothesis mkl>=2018 ninja numpy>=1.11 protobuf pytest setuptools six typing pyyaml psutil pillow
else
    retry pip install -qr requirements.txt || true
    retry pip install -q hypothesis protobuf pytest setuptools psutil numba==0.44.0 pillow || true
